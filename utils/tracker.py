    ...
    def _seed_initial_state(self) -> None:
        ...
        for file_path in self._iter_watched_files():
            try:
                ...
                # Expected for locked, temp, or permission-restricted files at startup
                log.debug("Could not stat file during initial seed (skipping): %s", file_path.name)

    def _iter_watched_files(self) -> "Generator[Path, None, None]":
