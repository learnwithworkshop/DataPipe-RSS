...
class GoogleSheetsConnector(BaseConnector):
    ...
    def send(self, article: ArticleRecord) -> bool:
        ...
        for attempt in range(1, self._max_retries + 2):
            ...
            # Progressive backoff before next attempt (skip after the final attempt)
            if attempt <= self._max_retries:
                backoff = 2 * attempt  # 2s after attempt 1, 4s after attempt 2
                log.debug("Google Sheets backoff: waiting %ds before retry.", backoff)
                time.sleep(backoff)
